...
学习流程：
```
记单词主页 → [开始学习] → 3a 学习卡片（认识/不认识）
    → [点击任一按钮] → 3b 释义页（释义+例句+下一词）
    → [下一词] → 3a（下一个单词，循环）
```
...
│ 💡 每日一词       来自 xxx  │
│ ┌─────────────────────────┐ │
│ │      ephemeral          │ │  ← 单词居中（Source Serif 4）
│ │ [美式] /ɪˈfemərəl/ [🔊]│ │  ← 英/美切换 + 发音按钮
│ │   adj. 短暂的，转瞬即逝  │ │  ← 词性 + 释义
│ │ "Fame is ephemeral..."  │ │  ← 例句
│ │   名声是短暂的           │ │  ← 翻译
│ └─────────────────────────┘ │
├─────────────────────────────┤
│      [📷 拍照] [✏️ 记录]    │  ← 底部紧凑 pill
...
- 单词 + 音标 + 词性标签
- 释义列表 + 例句
- "下一词 →" 按钮 → 回到 3a（下一个单词，循环）

**SRS 判定**：
- 新词学习时：认识=加入复习队列，不认识=稍后再学
- 旧词复习时：认识=reviewCount+1，不认识=归零回队尾
- 连续 10 次认识 = 掌握
